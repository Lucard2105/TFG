...
def CalcularPuntuacionesPartido(db, equipo_local, equipo_visitante, modo="Estandar"):
    ...
    try:
        Partido = eventos.find_one({"NombreEquipoLocal": equipo_local, "NombreEquipoVisitante": equipo_visitante})
        if not Partido:
            return {
                "error": f"No se encontró el partido entre {equipo_local} y {equipo_visitante}."
            }
    except Exception as e:
        return {
    ...
    try:
        Jjugadoreslocal = jugadores.find({"Equipo": equipo_local}, {"_id": 0, "Identificador": 1})
        Jjugadoresvisitante = jugadores.find({"Equipo": equipo_visitante}, {"_id": 0, "Identificador": 1})
        AjugadoresLocal = {}
        ...
        for jugador in AjugadoresLocal:
            ...
            puntos = int(puntuacionPartido(stats, modo)) + puntos_extra_local
            AjugadoresLocal[jugador] = puntos

        for jugador in AjugadoresVisitante:
            stats = AjugadoresVisitante[jugador]["Estadísticas"]
            puntos = int(puntuacionPartido(stats, modo)) + puntos_extra_visitante
...
# PuntosTotales = CalcularPuntuacionesPartido(db, "Quesos Hidalgo Manzanares FS", "Servigroup Peñíscola FS", "FairPlay")
# print(PuntosTotales)
